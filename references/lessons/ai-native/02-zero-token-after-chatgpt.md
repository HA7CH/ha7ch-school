...
## 为什么要在乎（钩子展开）
先回扣第 1 课，再戳它的漏洞：
> "上节课我们说：让用户的 agent 干活、产品接住结果。但有一个前提没被说出来——**它假设人人都有 agent workspace**——现实里，只有开发者才装了 Codex/Claude Code、会开终端、懂 npx。普通用户不会为了用一个小产品去装编程 agent。所以第 1 课其实只是'开发者的设计'。这节课讲那块缺失的拼图是怎么补上的。"
