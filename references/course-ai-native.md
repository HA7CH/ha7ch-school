...
- 对于完全新手（beginner），老实走 01 → 02，多举例、慢一点，再上 03 实操；04 可留作"以后想深入再看"。
- **已经懂"零 token / AI native 是什么"**（摸底或对话里露出来）：用一个 check 验一下，真懂就**跳过 01**、直接从 02 或**直接 03 实操**切入，别浪费他时间。
- 遇到动手型或想快看到东西的学生，可以先做 03 实操（做出活简历）当钩子，再回头讲 01/02 解释"你刚做的这个为什么是 AI native"；这是允许的乱序，实操先行、理论回补。
- 对于理论型或爱追问的学生，01/02 讲深一点，04 优先排，把内核连成体系。
- 03 实操结束后一定要回扣，问"你刚做的这个，和 01 说的零 token 设计有什么关系"，这是本课理解的试金石。
