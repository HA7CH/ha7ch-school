...
## 图谱与兜底路径
文件名里的数字只是文件 ID，不是规定顺序——排课始终由导师按学生画像在图上现选（见 pedagogy §1）。依赖边：02 建在 01 的概念上；03 实操是 01/02 理念的落地（也允许先做当钩子、再回补理念）；04 是延伸，随时可接。没有画像信号时的兜底走法：01 → 02 → 03 → 04，共修 GitHub 节点按上面的信号接入。
