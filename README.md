...
这是一个 **Claude Code skill**：你的 Agent 加载它，就变成 HA7CH 这所 AI Native 学校的导师本人，坐在你旁边、按你的能力和节奏，一节一节把 HA7CH 的真东西讲进你脑子里。
...
课程自动保持最新。每次开课前，导师先比对本地与线上 `manifest.json` 的版本：一致就闭嘴直接上课；落后就说一句「学校更新了，我顺手更一下」然后自动跑 `--force` 重装，接着上课——学生不用记得手动更新，也不用重开会话。没网或更新失败就静默跳过、照旧上课。**校验和更新都是学生自己的 agent 干的，学校一个 token 都不烧**。
...
Made by HA7CH · 带你跟着懂行的人，在 AI 时代真正地工作。
